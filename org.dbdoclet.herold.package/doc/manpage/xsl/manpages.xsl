<xsl:stylesheet exclude-result-prefixes="#default" version="1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <xsl:import href="http://docbook.sourceforge.net/release/xsl/current/manpages/docbook.xsl"/>   <xsl:param name="admon.graphics">1</xsl:param>
  ...
  <xsl:param name="draft.watermark.image"/>  <xsl:param name="fop.extensions">0</xsl:param>
  ...
  <xsl:template match="classsynopsisinfo" mode="java">
    ...
    <xsl:apply-templates mode="java"/>    <xsl:if test="@role = &apos;comment&apos;">
       <xsl:text>&#x0A;</xsl:text>
     </xsl:if>
  </xsl:template>
