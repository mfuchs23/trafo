<xsl:stylesheet exclude-result-prefixes="#default" version="1.0" xmlns:fo="http://www.w3.org/1999/XSL/Format" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <xsl:import href="http://docbook.sourceforge.net/release/xsl/current/epub/docbook.xsl"/>  <xsl:include href="http://dbdoclet.org/xsl/html/synopsis.xsl"/>  <xsl:include href="http://dbdoclet.org/xsl/html/themes/color.xsl"/>   <xsl:param name="admon.graphics">0</xsl:param>
  ...
  <xsl:param name="generate.toc"/>  <xsl:param name="glossary.collection"/>  <xsl:param name="glossterm.auto.link">0</xsl:param>
  ...
  <xsl:param name="ulink.target"/>  <xsl:param name="use.id.as.filename">0</xsl:param>
  ...
  <xsl:template name="container">
    <xsl:call-template name="write.chunk">
       <xsl:with-param name="filename">
         <xsl:value-of select="$epub.metainf.dir"/>         <xsl:value-of select="$epub.container.filename"/>       </xsl:with-param>
       <xsl:with-param name="method" select="&apos;xml&apos;"/>       <xsl:with-param name="encoding" select="&apos;utf-8&apos;"/>       <xsl:with-param name="indent" select="&apos;no&apos;"/>       <xsl:with-param name="quiet" select="$chunk.quietly"/>       <xsl:with-param name="doctype-public" select="&apos;&apos;"/>       <!-- intentionally blank -->
           ...
           <xsl:element name="rootfiles" namespace="urn:oasis:names:tc:opendocument:xmlns:container">
             <xsl:element name="rootfile" namespace="urn:oasis:names:tc:opendocument:xmlns:container">
               <xsl:attribute name="full-path">
                 <xsl:value-of select="&apos;OEBPS/content.opf&apos;"/>               </xsl:attribute>
